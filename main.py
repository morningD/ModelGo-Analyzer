...
# Create a graph and load the existing Turtle file
g = Graph()
g.parse("vocabulary.ttl", format="ttl")

# Create a new instance (resource)
new_resource = URIRef("http://example.org/Resource2")
g.add((new_resource, RDF.type, ex.Document))
g.add((new_resource, dcterms.title, Literal("New Resource")))
g.add((new_resource, dcterms.creator, Literal("Bob")))

# Save the updated graph to a new Turtle file
g.serialize(destination="updated.ttl", format="ttl")
